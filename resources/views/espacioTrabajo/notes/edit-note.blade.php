...
@section('content')
    ...
    <form action="{{ route('note.edit.process', ['id'=>$comision->com_id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="w-4/5 lg:h-[25rem] xl:h-[30rem] mt-4 p-4 flex flex-col lg:flex-row gap-x-6 bg-white rounded-md shadow-md shadow-negro/30">
                ...
                <div class="lg:w-1/2 pt-5 lg:pt-0 lg:flex lg:flex-col lg:justify-between border-t-2 border-rclaro lg:border-t-0">
                    <div class="h-[206px] flex-1">
                        @if ($noteDets->image != Null)
                            <img id="preview" src="{{ Storage::url($noteDets->image) }}" class="w-full h-full object-cover object-top rounded-md shadow-md shadow-negro/30 bg-rclaro/20">
                        @else
        ...
        <div class="my-10 flex w-full justify-center">
            <div class="w-4/5 flex justify-center">
                <button class="btn-principal" x-ref="btn">Guardar Nota</button>
            </div>
        </div>
    </form>
@endsection
